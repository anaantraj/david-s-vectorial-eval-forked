...
#: Which run the site describes. The defaults reproduce the first study, so an
#: unqualified `python experimental-notes/build.py` still rebuilds it byte for
#: byte. The environment overrides exist because the second study scores a
#: different set of columns on a different split, and pointing at it by editing
#: these constants would make the two studies unbuildable from one checkout.
def _run_dir(env: str, default: Path) -> Path:
    """Resolve a run directory, treating a relative override as repo-relative."""
...
RUN = _run_dir("VECTORIAL_REPORT_RUN", ROOT / "runs" / "scaled")
RUN_SYS = _run_dir("VECTORIAL_REPORT_RUN_SYS", ROOT / "runs" / "scaled_sys")
OUT = _run_dir("VECTORIAL_REPORT_OUT", OUT)
#: Evaluation split. `heldout` merges val and test. The second study scores
#: `test`, because the trained methods select on val and heldout contains val.
SPLIT = os.environ.get("VECTORIAL_REPORT_SPLIT", "heldout")
...
COLORS = {
    ...
    # Second study. One hue per method, one lightness step per training variant,
    # so a reader can see the method and the variant separately in any figure.
    "aspect_prompt_claude": "#8e5aa8",
    "lora_lora_target_lm": "#1f6f6f",
    "lora_lora_target_lm_paired": "#2f8f8f",
    "lora_lora_target_lm_aspect": "#5fb0b0",
    "soft_prompt_sp_target_lm": "#8a5a2b",
    "soft_prompt_sp_target_lm_paired": "#b07a3c",
    "soft_prompt_sp_target_lm_aspect": "#d0a068",
    "steering_steer_target_lm": "#6a4c93",
    "steering_steer_target_lm_paired": "#8a6cb0",
    "steering_steer_target_lm_aspect": "#ab93cc",
}
LABELS = {
    "identity": "identity",
    ...
    "lora_lora_target_lm": "LoRA (topic only)",
    "lora_lora_target_lm_paired": "LoRA (retrieved source)",
    "lora_lora_target_lm_aspect": "LoRA (aspects)",
    "soft_prompt_sp_target_lm": "soft prompt (topic only)",
    "soft_prompt_sp_target_lm_paired": "soft prompt (retrieved source)",
    "soft_prompt_sp_target_lm_aspect": "soft prompt (aspects)",
    "steering_steer_target_lm": "steering (topic only)",
    "steering_steer_target_lm_paired": "steering (retrieved source)",
    "steering_steer_target_lm_aspect": "steering (aspects)",
}
...
#: Column order, references first and last so systems are read between them.
#: Columns absent from a report are dropped by `load()`, so this one list serves
#: both studies.
ORDER = [
    "identity",
    "shuffle_control",
    ...
    "llm_rewrite_claude",
    "llm_fewshot_claude",
    "aspect_prompt_claude",
    "steering_steer_target_lm",
    "steering_steer_target_lm_paired",
    "steering_steer_target_lm_aspect",
    "soft_prompt_sp_target_lm",
    "soft_prompt_sp_target_lm_paired",
    "soft_prompt_sp_target_lm_aspect",
    "lora_lora_target_lm",
    "lora_lora_target_lm_paired",
    "lora_lora_target_lm_aspect",
    "target_sample",
]


def _report_path(run: Path) -> Path:
    """The report for SPLIT in this run, preferring the default embedding space.

    A run scored in a non-default space files its report under a slug, and a run
    may hold several. Values from two spaces are not comparable, so exactly one
    is chosen: the default name if present, otherwise the single slugged report,
    otherwise an error naming the candidates rather than picking one silently.
    """
    ...
    if len(slugged) == 1:
        return slugged[0]
    if not slugged:
        raise FileNotFoundError(f"no report for split {SPLIT!r} in {run}")
    raise RuntimeError(
        f"{run} holds {len(slugged)} reports for split {SPLIT!r}: "
        f"{[p.name for p in slugged]}. Values from two embedding spaces are not "
        f"comparable; remove or move the ones this site should not describe."
    )


def load():
    full = json.loads(_report_path(RUN).read_text())
    systems = json.loads(_report_path(RUN_SYS).read_text())
    manifest = json.loads((ROOT / "data" / "manifest.json").read_text())
    # Narrow the column order to what this run actually scored. ORDER lists the
    # columns of both studies, so without this a figure would try to read an
    # output file the run never produced.
    global ORDER
    ORDER = [fn for fn in ORDER if fn in full["table"]]
    missing = sorted(set(full["table"]) - set(ORDER))
    if missing:
        raise RuntimeError(
            f"{_report_path(RUN).name} scores columns absent from ORDER: {missing}. "
            f"Add them to ORDER, COLORS and LABELS rather than letting a figure "
            f"silently omit a system."
        )
...
#: Approximate glyph width per class, used to wrap figure text so that it
#: cannot run past the viewBox edge. The figure fonts are monospace, so a
#: fixed ratio is accurate enough for line breaking.
_CHAR_W = {
    "fig-title": 16 * 0.56, "fig-sub": 12 * 0.61, "fig-note": 11 * 0.61,
    "lbl": 12.5 * 0.61, "val": 11.5 * 0.61, "tick": 10.5 * 0.61,
    "tiny": 10.5 * 0.61, "key-lbl": 11.5 * 0.61, "pt-lbl": 12 * 0.61,
    "ax-title": 11.5 * 0.61, "zone-lbl": 11 * 0.61, "ref-lbl": 11 * 0.61,
}


def wrap_lines(s: str, width_px: float, cls: str) -> list[str]:
    """Break `s` into lines that fit within `width_px`."""
    cw = _CHAR_W.get(cls, 7.0)
    limit = max(8, int(width_px / cw))
...
def text_block(x, y, s, cls="fig-sub", width_px=740, line_h=14, anchor="start"):
    """Emit a wrapped run of text. Returns (svg, y_after)."""
    out = []
    for i, line in enumerate(wrap_lines(s, width_px, cls)):
        out.append(text(x, y + i * line_h, line, cls, anchor))
    return "\n".join(out), y + max(1, len(wrap_lines(s, width_px, cls))) * line_h
...
def scatter_tradeoff(systems, full, width=760, height=500):
    ...
    blk, _ = text_block(
        0, 38,
        "Filled circles are systems. Rings are reference points that sit at one end of an "
        "axis by definition. Bars show how much each value varies across cells.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
    ...
    blk, _ = text_block(
        0, height - 30,
        "The oracle scores near zero on keeping the content because it emits a different "
        "authentic post rather than a rewrite of the source.",
        "fig-note", width_px=width - 8, line_h=13,
    )
    out.append(blk)
    out.append("</svg>")
    return "\n".join(out)
...
def scorecard(systems, full, width=760):
    ...
    blk, _ = text_block(
        0, 38,
        "Filled proportion of each bar is performance on that axis, normalised across the "
        "columns shown. No column is strong on all three.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
...
def worst_features(full, width=760, top_n=8):
    ...
    blk, _ = text_block(
        0, 38,
        "How far each column moved a habit, compared with how far real Reddit posts sit "
        "from real LinkedIn posts. Read each column against the oracle, which shows how "
        "much this measure moves from sampling alone. Lower is better.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
...
def example_topic_jsd_failure():
    ...
    report = json.loads(_report_path(RUN_SYS).read_text())
...
#: (report key, plain label, metric family, reference).
#:
#: The label states what is measured and the metric name is kept in brackets so a
#: value can be traced to the code. `reference` applies to the diagnostic rows,
#: where neither the highest nor the lowest value is the goal and the aim is to
#: land on a particular level instead. "oracle" means the level real target posts
#: reach; a number means a fixed level. On those rows the mark shows the value
#: closest to that reference rather than the extreme.
TABLE_ROWS = [
    ("classifier.calibration_gap", "Reads like Reddit (calibration gap)", "classifier", None),
    ("classifier.target_rate", "Share judged Reddit (target rate)", "classifier", "oracle"),
    ("semantic.source_similarity", "Keeps the source content (source similarity)", "semantic", None),
    ("semantic.content_word_retention", "Keeps the specific details (content-word retention)", "semantic", None),
    ("structural.mean_feature_jsd", "Writing-habit mismatch (mean feature JSD)", "structural", None),
    ("structural.feature_coverage", "Writing habits reproduced (feature coverage)", "structural", None),
    ("trm.trm", "Matches the variety of real posts (TRM)", "trm", None),
    ("trm.trm_style_residual", "Same, with writing style removed (TRM, style-residual)", "trm", None),
    ("trm.trm_pvalue", "Chance of being mistaken for real (TRM p-value)", "trm", None),
    # Under a perfect match each of the three arrangements occurs one third of
    # the time, so one third is the target rather than zero or one.
    ("trm.rank_i2", "Outputs bunched together (rank I₂)", "trm", 1.0 / 3.0),
    ("distributional.mmd2", "Distance between the two sets (MMD²)", "distributional", None),
    ("distributional.topic_jsd", "Subject-matter mismatch (topic JSD)", "distributional", None),
    ("degeneracy.copy_rate", "Copied the source outright (copy rate)", "degeneracy", None),
    ("degeneracy.pool_self_similarity", "Outputs resemble each other (pool self-similarity)", "degeneracy", None),
    ("degeneracy.distinct_2", "Variety of phrasing used (distinct-2)", "degeneracy", None),
]
...
def resolution_table(systems, full, replication=None):
    """State which comparisons the bootstrap intervals actually separate.

    `replication` optionally maps a comparison label to its cross-space verdict
    ("stable" / "TF-IDF only" / "mixed"), adding a column that flags whether the
    verdict below survives a change of embedding space.
    """
    show_rep = bool(replication)
    rows = []
    for label, a, b, key in CLAIMS:
        ...
        rep_cell = ""
        if show_rep:
            tag = replication.get(label, "")
            tag_cls = "res" if tag == "stable" else "thin" if tag else ""
            chip = f'<span class="ncell {tag_cls}">{tag}</span>' if tag else "—"
            rep_cell = f"<td>{chip}</td>"
        rows.append(
            ...
            f'<td class="verdict {cls}">{verdict}</td>{rep_cell}</tr>'
        )
    rep_head = '<th scope="col">Across spaces</th>' if show_rep else ""
    rep_note = (
        " The final column states whether the verdict survives a change of embedding space; "
        "see the <a href=\"embedding-space.html\">embedding-space page</a>."
        if show_rep else ""
    )
    return (
        '<figure class="tablewrap wide"><table class="resolve"><caption>Every comparison stated in '
        ...
        f'<th scope="col">Second value</th><th scope="col">Verdict</th>{rep_head}</tr></thead>'
...
def results_table(report, fns, caption, base_fns=None):
    ...
    rows, footnoted, any_ref = [], False, False
    for key, label, metric, reference in TABLE_ROWS:
        score = key.split(".", 1)[1]
        base = None
        for f in base_fns:
        ...
        if direction:
            ...
            # Diagnostic row: the goal is to land on a reference level, so the
            # mark goes to whichever column comes closest to it.
            target = (
                values["target_sample"][0]
                if reference == "oracle" and "target_sample" in values
                else (reference if isinstance(reference, float) else None)
            )
            if target is not None:
                candidates = {f: m for f, (m, _n) in values.items() if f != "target_sample"} \
                    if reference == "oracle" else {f: m for f, (m, _n) in values.items()}
                if candidates:
                    best = min(candidates.values(), key=lambda m: abs(m - target))
                    any_ref = True
    ...
    if any_ref:
        key_note += (
            " On the two rows marked <span class=\"dir\">·</span> neither the highest nor "
            "the lowest value is the goal, so the mark shows the value closest to the "
            "level a perfect system would reach."
        )
...
# --------------------------------------------------------------------------
#
# Several metrics are computed inside an embedding space, and the project's
# stated central risk is that a general-purpose embedding may arrange posts by
# writing style rather than by subject. The same generated posts are therefore
# scored a second time in a different space and the two sets of numbers are
# compared here. Nothing about the outputs changes between the two; only the
# space in which they are measured does.

#: The spaces to compare, in a fixed display order. The first is the offline
#: default the main results pages use (its report keeps the unqualified filename,
#: so its slug is ``None``); the rest are neural, spanning two model families and,
#: for EmbeddingGemma, four Matryoshka widths. Each entry is
#: ``(key, short-label, report-slug)``. A space is only shown if both its
#: systems-only and full reports exist, so the page degrades to whatever the
#: sweep produced.
SPACE_SEQUENCE = [
    ("tfidf", "TF-IDF + SVD", None),
    ("minilm-384", "MiniLM-L6 384-d", "all-minilm-l6-v2-d384"),
    ("gemma-128", "Gemma 128-d", "embeddinggemma-300m-d128"),
    ("gemma-256", "Gemma 256-d", "embeddinggemma-300m-d256"),
    ("gemma-512", "Gemma 512-d", "embeddinggemma-300m-d512"),
    ("gemma-768", "Gemma 768-d", "embeddinggemma-300m-d768"),
]

#: The neural space the two-column figures (slopes, style gap, side-by-side
#: table) use as the reference. EmbeddingGemma at 512 dimensions is chosen over
#: 256 because 256 is the one neural width where the wrong-topic control is not
#: cleanly ranked worst on the triangle score (see the baseline-separation
#: figure), while 512 and 768 agree and 512 is the cheaper of the two.
NEURAL_REF_KEY = "gemma-512"

TFIDF_LABEL = "TF-IDF + SVD"
SPACE_COLORS = {"a": "#8b8b8b", "b": "#4a6fa5"}


def _load_pair(slug):
    name = f"report.{SPLIT}.json" if slug is None else f"report.{SPLIT}.{slug}.json"
    sp, fp = RUN_SYS / name, RUN / name
    if not (sp.exists() and fp.exists()):
        return None
    return json.loads(sp.read_text()), json.loads(fp.read_text())


def load_spaces():
    """Every configured space whose reports are present, in display order.

    Returns a list of dicts with ``key``, ``label``, ``systems`` and ``full``.
    """
    spaces = []
    for key, label, slug in SPACE_SEQUENCE:
        pair = _load_pair(slug)
        if pair is not None:
            spaces.append({"key": key, "label": label, "systems": pair[0], "full": pair[1]})
    return spaces


def load_alt():
    """The single neural reference space used by the two-column figures.

    Falls back to the last available neural space, then to whatever is present,
    so the page still builds if the recommended width was not scored.
    """
    spaces = load_spaces()
    neural = [s for s in spaces if s["key"] != "tfidf"]
    ref = next((s for s in spaces if s["key"] == NEURAL_REF_KEY), None) or (
        neural[-1] if neural else None
    )
    if ref is None:
        raise FileNotFoundError(
            "No neural embedding report found. Run `evaluate` with "
            "--embedding-model google/embeddinggemma-300m before building."
        )
    return ref["full"], ref["systems"], ref["label"]


def _space_record(report) -> dict:
    """The space a report was computed in.

    Reports written before the space was recorded explicitly still carry the
    configuration that produced them, so both forms are read.
    """
    space = report.get("embedding_space")
    if isinstance(space, dict) and space:
        return space
    cfg = report.get("config", {}).get("eval", {})
    backend = cfg.get("embedding_backend")
    return {
        "backend": backend,
        "model": cfg.get("embedding_model") if backend != "tfidf-svd" else backend,
        "dim": cfg.get("embedding_dim"),
    }


def space_name(report) -> str:
    space = _space_record(report)
    return space.get("model") or space.get("backend") or "unknown"


def space_dim(report):
    return _space_record(report).get("dim")


def split_rows_by_space_dependence(rep_a, rep_b, fns):
    """Which table rows the space moves, and which it leaves untouched.

    Computed rather than asserted. The rows it leaves untouched are the control:
    they are measured without an embedding, so if any of them moved, the two runs
    would differ in something other than the space and no comparison below would
    mean anything.
    """
    moved, fixed = [], []
    for row in TABLE_ROWS:
        key = row[0]
        pairs = [
            (val(rep_a, f, key), val(rep_b, f, key))
            for f in fns
            if val(rep_a, f, key) is not None and val(rep_b, f, key) is not None
        ]
        if not pairs:
            continue
        shift = max(abs(a - b) for a, b in pairs)
        (moved if shift > 1e-9 else fixed).append((row, shift))
    return moved, fixed


def ordering(rep, fns, key, direction):
    """System columns ordered best to worst on `key`, or None if unscorable."""
    vals = [(f, val(rep, f, key)) for f in fns]
    vals = [(f, v) for f, v in vals if v is not None]
    if len(vals) < 2:
        return None
    return [f for f, _ in sorted(vals, key=lambda t: t[1] * (-1 if direction == 1 else 1))]


def _spread(items, min_gap, lo, hi):
    """Push labels apart to `min_gap` and keep the run inside `lo`..`hi`.

    Returns key -> y. Ties in the input are broken by the key so that the
    result does not depend on dictionary ordering.
    """
    order = sorted(items, key=lambda t: (t[1], t[0]))
    ...
    return {key: y for (key, _), y in zip(order, ys, strict=True)}


def space_slopes(rep_a, rep_b, keys, fns, alt_label="EmbeddingGemma", width=760):
    """Small multiples: each system's value in one space joined to the other.

    A panel whose lines stay parallel says the space rescaled the measure without
    reordering the columns. Crossing lines say the space changed which column
    comes out ahead, which is the only kind of disagreement that would alter a
    conclusion.
    """
    alt_tick = alt_label.split(" ")[0].replace("-IDF", "-IDF")  # "Gemma" / "MiniLM-L6"
    subtitle = (
        f"One line per column. The left end is the value measured with TF-IDF, the right "
        f"end the value measured with {alt_label}. Each panel is scaled to its own "
        "range, so the useful thing to read is whether lines cross, not how steep they are."
    )
    cols, panel_w, panel_h = 2, width // 2, 214
    rows_n = (len(keys) + cols - 1) // cols
    head = 38 + 14 * len(wrap_lines(subtitle, width - 8, "fig-sub")) + 12
    h = head + rows_n * panel_h + 34

    out = [svg_open(width, h, "Metric values in both embedding spaces")]
    out.append(text(0, 20, "Each measure in both spaces", "fig-title"))
    blk, _ = text_block(0, 38, subtitle, "fig-sub", width_px=width - 8)
    out.append(blk)

    for i, (key, label, _metric, _ref) in enumerate(keys):
        px0 = (i % cols) * panel_w
        py0 = head + (i // cols) * panel_h
        pad_l, pad_r, pad_t, pad_b = 34, 162, 40, 40
        pw = panel_w - pad_l - pad_r
        ph = panel_h - pad_t - pad_b

        series = [
            (f, val(rep_a, f, key), val(rep_b, f, key))
            for f in fns
            if val(rep_a, f, key) is not None and val(rep_b, f, key) is not None
        ]
        if not series:
            continue
        # Limits come from the values plotted, never from a fixed number.
        observed = [v for _, a, b in series for v in (a, b)]
        lo, hi = min(observed), max(observed)
        span = (hi - lo) or 1.0
        lo, hi = lo - span * 0.14, hi + span * 0.14

        def ypos(v, py0=py0, pad_t=pad_t, ph=ph, lo=lo, hi=hi):
            return py0 + pad_t + ph * (1 - (v - lo) / (hi - lo))

        x_a, x_b = px0 + pad_l, px0 + pad_l + pw
        for j, line in enumerate(wrap_lines(label, panel_w - 16, "lbl")[:2]):
            out.append(text(px0, py0 + 16 + 14 * j, line, "lbl"))
        out.append(f'<line class="axis" x1="{x_a}" y1="{py0+pad_t-8}" x2="{x_a}" y2="{py0+pad_t+ph+8}"/>')
        out.append(f'<line class="axis" x1="{x_b}" y1="{py0+pad_t-8}" x2="{x_b}" y2="{py0+pad_t+ph+8}"/>')
        out.append(text(x_a, py0 + pad_t + ph + 26, "TF-IDF", "tick", "middle"))
        out.append(text(x_b, py0 + pad_t + ph + 26, alt_tick, "tick", "middle"))

        # Labels are separated before anything is drawn and kept inside the
        # panel, so that a pair of close values cannot print over one another
        # and a crowded panel cannot push its last label onto the axis caption.
        lo_y, hi_y = py0 + pad_t - 4, py0 + pad_t + ph + 8
        left_y = _spread([(fn, ypos(a) + 4) for fn, a, _ in series], 12, lo_y, hi_y)
        right_y = _spread([(fn, ypos(b) + 4) for fn, _, b in series], 13, lo_y, hi_y)

        for fn, a, b in sorted(series, key=lambda t: -t[2]):
            colour = COLORS.get(fn, "#888")
            ya, yb = ypos(a), ypos(b)
            out.append(
                f'<line class="slope" x1="{x_a:.1f}" y1="{ya:.1f}" x2="{x_b:.1f}" '
                f'y2="{yb:.1f}" stroke="{colour}" stroke-width="2" fill="none"/>'
            )
            out.append(f'<circle class="pt" cx="{x_a:.1f}" cy="{ya:.1f}" r="4" fill="{colour}"/>')
            out.append(f'<circle class="pt" cx="{x_b:.1f}" cy="{yb:.1f}" r="4" fill="{colour}"/>')
            out.append(text(x_a - 7, left_y[fn], f"{a:.2f}", "tiny", "end"))
            out.append(text(x_b + 8, right_y[fn], f"{b:.2f}  {LABELS.get(fn, fn)}", "tiny"))
    out.append("</svg>")
    return "\n".join(out)


def style_gap_bars(rep_a, rep_b, fns, alt_label="EmbeddingGemma", width=760):
    """How much of each TRM value the style directions account for, per space.

    The gap is the raw group-similarity score minus the score with the
    platform-separating directions removed. A large gap means the measurement was
    largely about writing style. Whether that gap shrinks in a stronger space is
    ...
    for fn in fns:
        ...
        for rep in (rep_a, rep_b):
            raw, res = val(rep, fn, "trm.trm"), val(rep, fn, "trm.trm_style_residual")
            pair.append(None if raw is None or res is None else raw - res)
        if any(v is not None for v in pair):
            series.append((fn, pair[0], pair[1]))
    if not series:
        return ""

    subtitle = (
        "The group-similarity score minus the same score with the two directions that "
        "separate the platforms removed. A long bar means the measurement was largely "
        "responding to how the post is written rather than to what it is about. Bars are "
        "drawn by size; a negative value means the score rose slightly once the two "
        "directions were removed."
    )
    row_h, left, gap = 44, 224, 7
    # The header grows with the subtitle rather than being fixed, so the legend
    # cannot end up printed over the last line of it.
    top = 38 + 14 * len(wrap_lines(subtitle, width - 8, "fig-sub")) + 40
    h = top + row_h * len(series) + 46
    plot_w = width - left - 78
    observed = [abs(v) for _, a, b in series for v in (a, b) if v is not None]
    vmax = (max(observed) or 1.0) * 1.25

    out = [svg_open(width, h, "Share of the group-similarity score attributable to style")]
    out.append(text(0, 20, "How much of the score is writing style", "fig-title"))
    blk, _ = text_block(0, 38, subtitle, "fig-sub", width_px=width - 8)
    out.append(blk)
    space_labels = {"a": TFIDF_LABEL, "b": alt_label}
    for j, k in enumerate(("a", "b")):
        out.append(f'<rect class="key" x="{left + j*250}" y="{top-32}" width="11" height="11" fill="{SPACE_COLORS[k]}"/>')
        out.append(text(left + j * 250 + 17, top - 22, space_labels[k], "key-lbl"))

    for i, (fn, a, b) in enumerate(series):
        y = top + i * row_h
        out.append(text(left - 10, y + 19, rlabel(fn), "lbl", "end"))
        for j, (v, k) in enumerate(((a, "a"), (b, "b"))):
            yy = y + j * (14 + gap)
            if v is None:
                out.append(text(left + 4, yy + 12, "not scored", "muted"))
                continue
            bw = max(1.5, plot_w * (abs(v) / vmax))
            out.append(
                f'<rect class="bar" x="{left}" y="{yy}" width="{bw:.1f}" height="14" '
                f'fill="{SPACE_COLORS[k]}" rx="2"/>'
            )
            out.append(text(left + bw + 6, yy + 12, f"{v:+.3f}", "val"))
    out.append(f'<line class="axis" x1="{left}" y1="{top-6}" x2="{left}" y2="{h-32}"/>')
    out.append("</svg>")
    return "\n".join(out)


def space_table(rep_a, rep_b, fns, moved, caption, alt_label="EmbeddingGemma"):
    """Both spaces, row by row, with the ordering of the columns stated."""
    space_labels = {"a": TFIDF_LABEL, "b": alt_label}
    head = "".join(
        f'<th class="{"base" if f in BASELINE_FNS else "sys"}">{esc(LABELS.get(f, f))}'
        f'<span class="coltag">{ROLES.get(f, "system")}</span></th>'
    ...
    body = []
    for (key, label, _metric, _ref), _shift in moved:
        entry0 = next(
            (rep_a["table"][f].get(key) for f in fns if rep_a["table"].get(f, {}).get(key)), None
        )
        direction = entry0.get("direction", 0) if entry0 else 0
        order_a = ordering(rep_a, fns, key, direction)
        order_b = ordering(rep_b, fns, key, direction)
        same = order_a is not None and order_a == order_b
        agree = (
            f'<span class="ncell{"" if same else " thin"}">'
            f'{"same order" if same else "order changes"}</span>'
        )
        for j, (rep, k) in enumerate(((rep_a, "a"), (rep_b, "b"))):
            cells = []
            for f in fns:
                v = val(rep, f, key)
                cls = "base" if f in BASELINE_FNS else "sys"
                cells.append(f'<td class="{cls}">{"—" if v is None else f"{v:.3f}"}</td>')
            if j == 0:
                lead = (
                    f'<th scope="row" rowspan="2">{esc(label)} '
                    f'<span class="dir">{ARROW.get(direction, "·")}</span>{agree}</th>'
                )
            else:
                lead = ""
            body.append(
                f'<tr class="{"spacetop" if j == 0 else "spacebot"}">{lead}'
                f'<td class="spacelbl">{esc(space_labels[k])}</td>'
                + "".join(cells) + "</tr>"
            )

    return (
        f'<figure class="tablewrap wide"><table class="spaces"><caption>{caption}</caption>'
        f'<thead><tr><th scope="col">Metric</th><th scope="col">Space</th>{head}</tr></thead>'
        f"<tbody>{''.join(body)}</tbody></table>"
        '<p class="tablekey">Each measure is shown twice, once per space. The note beside '
        "each measure states whether the columns come out in the same order in both spaces. "
        "An order that changes is the only difference that could alter a conclusion; a "
        "change in the numbers alone is expected, because the two spaces are different "
        "scales and their values were never comparable.</p></figure>"
    )


def _verdict(rep_a_first, rep_a_second, a, b, key):
    ea = rep_a_first["table"].get(a, {}).get(key)
    eb = rep_a_second["table"].get(b, {}).get(key)
    if not ea or not eb:
        return None
    overlap = not (ea["ci_high"] < eb["ci_low"] or ea["ci_low"] > eb["ci_high"])
    return ("not resolved" if overlap else "supported"), ea, eb


#: Short column headers for the six-space matrix, keyed on the sequence key.
SHORT_COL = {
    "tfidf": "TF-IDF", "minilm-384": "MiniLM", "gemma-128": "G-128",
    "gemma-256": "G-256", "gemma-512": "G-512", "gemma-768": "G-768",
}


def claim_verdict(space, a, b, key):
    """Supported / not-resolved for one comparison within one space, or None."""
    systems, full = space["systems"], space["full"]
    r1 = full if a == "target_sample" else systems
    r2 = full if b == "target_sample" else systems
    res = _verdict(r1, r2, a, b, key)
    return None if res is None else res[0]


def resolution_matrix(spaces, width=760):
    """Every stated comparison against every space, one column per space.

    The point the table makes on its own: the two comparisons that hold under
    TF-IDF and under no neural space stand out as a block of otherwise-uniform
    rows. Verdicts are computed, never typed.
    """
    rows = []
    n_unanimous = tfidf_only = 0
    tfidf_only_labels = []
    per_claim = {}
    for label, a, b, key in CLAIMS:
        verdicts = [claim_verdict(s, a, b, key) for s in spaces]
        present = [v for v in verdicts if v is not None]
        if not present:
            continue
        unanimous = len(set(present)) == 1
        tfidf_v = verdicts[0]
        neural = [v for v in verdicts[1:] if v is not None]
        only = tfidf_v == "supported" and neural and all(v == "not resolved" for v in neural)
        if unanimous:
            n_unanimous += 1
        if only:
            tfidf_only += 1
            tfidf_only_labels.append(label)
        tag = ("stable" if unanimous else "TF-IDF only" if only else "mixed")
        per_claim[label] = tag
        tag_cls = "res" if unanimous else "thin"
        cells = "".join(
            f'<td class="verdict {"res" if v == "supported" else "unres" if v == "not resolved" else ""}">'
            f'{("supp." if v == "supported" else "not res." if v == "not resolved" else "—")}</td>'
            for v in verdicts
        )
        rows.append(
            f'<tr><th scope="row">{esc(label)}</th>{cells}'
            f'<td><span class="ncell {tag_cls}">{tag}</span></td></tr>'
        )
    head = "".join(f'<th scope="col">{esc(SHORT_COL.get(s["key"], s["label"]))}</th>' for s in spaces)
    caption = (
        f"Every comparison the report states, tested in all {len(spaces)} spaces. "
        f"{n_unanimous} reach the same verdict in every space. {tfidf_only} hold under "
        "TF-IDF and under no neural space, and are therefore not results. A comparison "
        "counts as supported only when the two bootstrap intervals do not overlap."
    )
    html = (
        f'<figure class="tablewrap wide"><table class="resolve spaces"><caption>{caption}</caption>'
        f'<thead><tr><th scope="col">Comparison</th>{head}<th scope="col">Across spaces</th></tr>'
        f"</thead><tbody>{''.join(rows)}</tbody></table>"
        '<p class="tablekey">Each column is the same outputs scored in one representation. '
        '<span class="verdict res">supp.</span> means the two intervals separate; '
        '<span class="verdict unres">not res.</span> means they overlap. The last column '
        "flags whether a comparison is stable across every representation, holds only under "
        "TF-IDF, or is split.</p></figure>"
    )
    return html, {
        "n_claims": len([1 for lab, a, b, k in CLAIMS if any(claim_verdict(s, a, b, k) for s in spaces)]),
        "n_unanimous": n_unanimous,
        "n_tfidf_only": tfidf_only,
        "tfidf_only_labels": tfidf_only_labels,
        "per_claim": per_claim,
    }


def baseline_separation(spaces, fns, width=760):
    """Where the wrong-topic control lands on the triangle score, per space.

    The harness rule is that a triangle score which ranks the wrong-topic control
    as well as a real system is measuring register rather than content and must
    not be reported. Each space gets one strip, scaled to its own range because
    values are not comparable across spaces; the dot for the wrong-topic control
    is emphasised and its rank among the columns is stated.
    """
    strips = []
    for sp in spaces:
        rep = sp["systems"]
        pts = [(f, val(rep, f, "trm.trm")) for f in fns]
        pts = [(f, v) for f, v in pts if v is not None]
        if len(pts) < 3 or "shuffle_control" not in dict(pts):
            continue
        rank = sorted(pts, key=lambda t: t[1])  # lower trm = closer to real = better
        order = [f for f, _ in rank]
        strips.append((sp["label"], pts, order.index("shuffle_control") + 1, len(order)))
    if not strips:
        return ""

    left, right, top = 128, 232, 150
    row_h = 44
    strip_w = width - left - right
    h = top + row_h * len(strips) + 20

    out = [svg_open(width, h, "Where the wrong-topic control lands on the triangle score")]
    out.append(text(0, 20, "Does the triangle score separate the baselines?", "fig-title"))
    blk, y = text_block(
        0, 38,
        "The shuffle control is real Reddit posts on the wrong topic. A score that measures "
        "subject matter must place it among the worst columns. A score that measures only "
        "register places it among the best, because its register is correct. Each row is one "
        "representation, scaled to its own range; every dot is one column's score, and the "
        "ringed dot is the wrong-topic control.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
    # Compact colour key for the non-control columns.
    keyfns = [f for f in fns if f != "shuffle_control"]
    kx = 0
    ky = y + 4
    for f in keyfns:
        out.append(f'<rect class="key" x="{kx}" y="{ky-9}" width="10" height="10" fill="{COLORS.get(f,"#888")}"/>')
        lbl = LABELS.get(f, f)
        out.append(text(kx + 15, ky, lbl, "tiny"))
        kx += 15 + len(lbl) * _CHAR_W["tiny"] + 18

    ax_y = top - 16
    out.append(text(left, ax_y, "closer to real Reddit (better)", "tiny"))
    out.append(text(left + strip_w, ax_y, "further", "tiny", "end"))

    for i, (label, pts, srank, n) in enumerate(strips):
        cy = top + i * row_h + row_h / 2
        vals = [v for _, v in pts]
        lo, hi = min(vals), max(vals)
        span = (hi - lo) or 1.0

        def px(v, lo=lo, span=span):
            return left + strip_w * (v - lo) / span

        out.append(text(left - 12, cy + 4, label, "lbl", "end"))
        out.append(f'<line class="grid" x1="{left}" y1="{cy:.1f}" x2="{left+strip_w}" y2="{cy:.1f}"/>')
        # Non-control columns first, control on top so its ring is never hidden.
        for f, v in pts:
            if f == "shuffle_control":
                continue
            out.append(f'<circle cx="{px(v):.1f}" cy="{cy:.1f}" r="4.5" fill="{COLORS.get(f,"#888")}"/>')
        cv = dict(pts)["shuffle_control"]
        out.append(
            f'<circle cx="{px(cv):.1f}" cy="{cy:.1f}" r="8" fill="#fff" '
            f'stroke="{COLORS["shuffle_control"]}" stroke-width="2.5"/>'
        )
        out.append(f'<circle cx="{px(cv):.1f}" cy="{cy:.1f}" r="2.6" fill="{COLORS["shuffle_control"]}"/>')
        verdict = "worst (good)" if srank == n else (
            "best (register)" if srank <= 2 else "mid")
        out.append(
            f'<text class="tiny" x="{left+strip_w+8}" y="{cy+4:.1f}" '
            f'text-anchor="start">rank {srank}/{n}, {esc(verdict)}</text>'
        )
    out.append("</svg>")
    return "\n".join(out)


def withdrawn_gap_table(spaces):
    """The identity-vs-Claude triangle gap in every space.

    The withdrawn claim is that Claude rewriting moves the pool further from real
    Reddit than leaving the source untouched. The gap is large and separates only
    under TF-IDF; in every neural space it is small and within noise. Absolute
    values are not comparable across spaces, so only the within-space gap and its
    separation are read.
    """
    rows = []
    for sp in spaces:
        r = sp["systems"]
        a = r["table"].get("identity", {}).get("trm.trm")
        b = r["table"].get("llm_rewrite_claude", {}).get("trm.trm")
        if not a or not b:
            continue
        sep = b["ci_low"] > a["ci_high"] or a["ci_low"] > b["ci_high"]
        rows.append(
            f'<tr><th scope="row">{esc(sp["label"])}</th>'
            f'<td class="num">{a["mean"]:.3f}</td><td class="num">{b["mean"]:.3f}</td>'
            f'<td class="num">{b["mean"]-a["mean"]:+.3f}</td>'
            f'<td class="verdict {"res" if sep else "unres"}">{"separates" if sep else "within noise"}</td></tr>'
        )
    return (
        '<figure class="tablewrap"><table class="resolve"><caption>The one withdrawn '
        "claim, measured in each space: does Claude rewriting move the pool further from "
        "real Reddit than the untouched source? Absolute triangle scores are not comparable "
        "across spaces, so only the within-space gap and whether the intervals separate are "
        'read.</caption><thead><tr><th scope="col">Space</th><th scope="col">source</th>'
        '<th scope="col">Claude</th><th scope="col">gap</th><th scope="col">Intervals</th>'
        f"</tr></thead><tbody>{''.join(rows)}</tbody></table></figure>"
    )
...
NAV = [
    ...
    ("embedding-space.html", "Embedding space"),
    ("limitations.html", "Limitations"),
]


def _build_stamp() -> datetime.datetime:
    """When the report says it was generated.

    The wall clock by default, so a published page states when it was built.
    That makes the build non-deterministic across a minute boundary, which
    defeats the byte-identical check AGENTS.md requires after any change to this
    file: two builds either side of a minute differ in one character of every
    page, and the check then reports a difference that is not a difference.

    `SOURCE_DATE_EPOCH` (the reproducible-builds convention, a Unix timestamp)
    pins the stamp so the check tests what it is meant to test. Nothing else in
    the output depends on the clock.
    """
    epoch = os.environ.get("SOURCE_DATE_EPOCH")
    if epoch:
        return datetime.datetime.fromtimestamp(int(epoch)).astimezone()
...
<footer class="foot"><div class="wrap">
  <p>Generated from <code>{esc(_report_path(RUN).relative_to(ROOT))}</code> and the
     matching systems-only report by
...
def build():
    ...
    # ---- embedding-space comparison, computed first ----------------------
    # Three pages quote counts from this comparison, so it is computed before
    # any of them is written rather than typed into each.
    spaces = load_spaces()
    full_alt, systems_alt, alt_label = load_alt()
    moved, fixed = split_rows_by_space_dependence(systems, systems_alt, sys_fns)
    reorder = [
        row for row, _ in moved
        if (lambda d: ordering(systems, sys_fns, row[0], d) != ordering(systems_alt, sys_fns, row[0], d))(
            next((systems["table"][f][row[0]].get("direction", 0)
                  for f in sys_fns if systems["table"].get(f, {}).get(row[0])), 0)
        )
    ]
    matrix_html, mstats = resolution_matrix(spaces)
    baseline_html = baseline_separation(spaces, sys_fns)
    gap_html = withdrawn_gap_table(spaces)
    n_claims = mstats["n_claims"]
    n_unanimous = mstats["n_unanimous"]
    n_withdrawn = mstats["n_tfidf_only"]
    withdrawn_labels = mstats["tfidf_only_labels"]
    n_neural = len(spaces) - 1
    # Coverage is a function of pool sizes rather than of the space, so the two
    # runs must cover the same cells. If they ever did not, the comparison would
    # be between different subsets and the page would be meaningless.
    same_coverage = systems["common_cells"] == systems_alt["common_cells"]
    fixed_labels = ", ".join(row[1].split(" (")[0].lower() for row, _ in fixed)

    # Claims made in the closing prose are computed here rather than asserted,
    # because a sentence that was true of an earlier run becomes false silently.
    trm_reorders = any(row[0] == "trm.trm" for row in reorder)
    llm_fns = [f for f in sys_fns if f.startswith("llm_")]

    def jsd_indistinct_from_control(rep):
        """Rewritten columns scoring no better than the wrong-topic control.

        Scoring level with a column that was handed the wrong topic entirely is
        the signature of the subject-matter measure documented as unreliable for
        generated text. The count is computed here so the prose below cannot
        outlive the run it describes.
        """
        control = val(rep, "shuffle_control", "distributional.topic_jsd")
        vals = [val(rep, f, "distributional.topic_jsd") for f in llm_fns]
        if control is None or any(v is None for v in vals):
            return None
        return sum(1 for v in vals if v >= control)

    jsd_a, jsd_b = jsd_indistinct_from_control(systems), jsd_indistinct_from_control(systems_alt)
    jsd_flips = jsd_a is not None and jsd_b is not None and jsd_a > 0 and jsd_b == 0

    # Is the style-removal difference small next to the score it is taken from?
    # A tenth is the threshold; the sentence it governs is written both ways.
    def _style_gap_ratio(rep):
        gaps, raws = [], []
        for f in sys_fns:
            raw, res = val(rep, f, "trm.trm"), val(rep, f, "trm.trm_style_residual")
            if raw is None or res is None:
                continue
            gaps.append(abs(raw - res))
            raws.append(abs(raw))
        return max(gaps) / max(raws) if gaps and max(raws) else None

    ratios = [_style_gap_ratio(systems), _style_gap_ratio(systems_alt)]
    style_gap_small = all(r is not None and r < 0.10 for r in ratios)
...
<section class="callout">
  ...
  <p>Both halves of this are now measured. The rewrites genuinely do read more like Reddit
  than the source posts. They also genuinely do lose the variety: the outputs use a
  narrower vocabulary, they are more alike one another than the source posts are, and they
  settle on a single structure, a closing question, used about three times as often as
  real Reddit posts use it.</p>
  ...
  the rest.</p>
  <p class="note">Some of these measurements are made inside a numerical representation of
  the posts. Repeated in six representations spanning two encoder families,
  {n_unanimous} of the {n_claims} stated comparisons reach the same verdict in every one;
  {n_withdrawn} hold only under the offline default and are not results. Which ones, and
  why, is set out on <a href="embedding-space.html">the embedding-space page</a>.</p>
</section>
...
<h2>Reads like Reddit per post; the group is too uniform</h2>
<p>Read one post at a time, the classifier reports that the Claude rewrites read more like
Reddit than the source posts do. The question the group-level measures ask is a different
one: does the <em>set</em> of rewrites resemble the <em>set</em> of real Reddit posts, or
has the model collapsed onto one average voice?</p>
{paired_bars(systems, full)}
<p class="note">The right-hand bar is the triangle score, which is computed inside an
embedding space. In the offline representation used for these tables it reports the
rewrites as far from real Reddit, but that representation ranks a wrong-topic control among
its best columns, so its triangle score is largely reading register rather than subject.
The gap disappears in every content-sensitive representation; see
<a href="embedding-space.html">the embedding-space page</a>. The durable evidence that the
group is too uniform is set out below, and none of it depends on the embedding.</p>
<p>The table states which stated comparisons the intervals support, and the final column
flags whether each verdict survives a change of representation.</p>
{resolution_table(systems, full, replication=mstats["per_claim"])}
<p>The two comparisons that do not replicate both rest on the triangle score in the offline
representation. They are set aside. What follows does not depend on any embedding.</p>

<h2>Where the variety is lost</h2>
<p>Three space-independent measurements establish the group-level problem directly, without
relying on the triangle score. The first is that the outputs are more alike one another
than the source posts are, and use a narrower vocabulary.</p>

{hbar_chart([(f, val(systems, f, "degeneracy.pool_self_similarity")) for f in sys_fns], "Rewrites resemble one another more than the source posts do", "How alike the posts within each set are; the source (identity) marks the untouched level.", ref=val(systems, "identity", "degeneracy.pool_self_similarity"), ref_label="source level")}
...
"""
    ...
    # ---- embedding space -------------------------------------------------
    withdrawn_prose = "; ".join(f"&ldquo;{esc(lbl.lower())}&rdquo;" for lbl in withdrawn_labels)
    emb = f"""
<h2>Why the space is checked</h2>
<p>Several of the measures in this report are not computed on the text directly. They
are computed on a numerical representation of each post, and the choice of
representation is a judgement rather than a detail. The risk the project identified at
the outset is that a general representation may arrange posts by how they are written
rather than by what they are about, in which case a measure that appears to compare
subject matter is really comparing tone.</p>
<p>The check is to measure the same posts in several representations and see what
survives. Nothing about the generated posts changes between runs. Only the representation
they are measured in changes. Six representations are compared: the offline word-count
default, a second sentence-encoder family, and one encoder at four sizes.</p>
<ul class="spec">
  <li><strong>TF-IDF + SVD</strong> at 256 dimensions. Word counts weighted by rarity and
  then compressed. It is fitted on this corpus, needs no download, and knows nothing about
  language beyond which words co-occur here. This is the offline default the results pages
  use.</li>
  <li><strong>EmbeddingGemma</strong>, a trained sentence encoder, at 128, 256, 512 and
  768 dimensions. It carries general knowledge of meaning and places posts near one another
  when they are about the same thing, whether or not they share vocabulary. The four sizes
  test whether a conclusion is stable within one encoder.</li>
  <li><strong>MiniLM-L6</strong> at 384 dimensions. A second, unrelated encoder family,
  included so that agreement is not a coincidence of one model.</li>
</ul>

<h2>Does the score separate the baselines?</h2>
<p>The harness has a built-in test for whether a triangle score is measuring content or
merely register. The shuffle control is real Reddit posts on the wrong topic. A score
that measures subject matter must rank it among the worst columns; a score that responds
only to register ranks it among the best, because its register is already correct. The
rule is that a score which cannot separate the baselines this way must not be reported.</p>
{baseline_html}
<p>The word-count representation fails this test: it ranks the wrong-topic control second
best, above all three rewriters, which means its triangle score is largely reading
register rather than subject. Every neural representation ranks the wrong-topic control
worst, where a content-sensitive score should, with EmbeddingGemma at 256 dimensions the
one marginal case. This is the reason the results pages report the neural triangle score
at 512 dimensions rather than 256.</p>

<h2>Which conclusions survive</h2>
{matrix_html}
<p>Of the {n_claims} comparisons the report states, {n_unanimous} reach the same verdict
in every representation. {n_withdrawn} hold under the word-count default and under no
neural representation: {withdrawn_prose}. Both rest on the triangle score in the one
representation that fails the baseline test above, so both are properties of that
representation rather than of the posts, and neither is a result.</p>

<h3>The withdrawn distribution-match claim, in detail</h3>
{gap_html}
<p>The gap between the untouched source and the Claude rewrite is large and its intervals
separate only under the word-count representation. In every neural representation, at every
size and in both model families, the gap shrinks to within noise. More dimensions do not
recover it, so the effect is not merely under-measured at 256 dimensions; it was specific
to the space that fails the baseline test.</p>

<h2>How each measure moves</h2>
<p>The remaining figures compare the word-count default against the neural reference
({esc(alt_label)}) directly. {"Both runs cover exactly the same cells on every measure" if same_coverage else "The two runs do not cover the same cells, so this comparison is not like for like"},
so the two sets of numbers are averaged over the same posts. Of the measures in the
results table, {len(moved)} are computed in the representation and {len(fixed)} are not.
The {len(fixed)} that are not come out identical in both runs, which confirms the runs
differ in the representation and in nothing else. Those measures are {esc(fixed_labels)}.</p>
{space_slopes(systems, systems_alt, [row for row, _ in moved], sys_fns, alt_label=alt_label)}
<p>The values themselves are expected to differ, because the two representations are
different scales and were never comparable. What matters is whether the columns come out
in the same order, since the order is what a conclusion rests on. The order changes on
{len(reorder)} of the {len(moved)} measures.</p>

{space_table(systems, systems_alt, sys_fns, moved, f"The word-count default against the neural reference ({esc(alt_label)}), measure by measure. The oracle is excluded so that all columns are scored on an identical set of cells.", alt_label=alt_label)}

<h2>How much of the score was writing style</h2>
<p>Every measure computed in a representation is also computed a second time with the two
directions that best separate LinkedIn from Reddit removed. The difference between the
two is an estimate of how much of the score was responding to writing style rather than
to subject matter.</p>
{style_gap_bars(systems, systems_alt, sys_fns, alt_label=alt_label)}
<p>{"In both representations the removal moves the score very little relative to the score itself, so neither appears to be measuring writing style to any great degree." if style_gap_small else "The removal moves the score substantially, so a large part of what is being measured is writing style rather than subject matter."}
This estimate is a lower bound in both representations, because two straight-line
directions cannot capture everything that makes a post read as one platform or the
other, and it is measured along different directions in each. The baseline-separation test
above is the more direct check, and it is what the word-count representation fails.</p>

<h2>Reading them together</h2>
<p>The triangle score is defined relative to the distance it is computed over, so a value
from one representation cannot be compared with a value from another at all.
{"It also ranks the columns differently across representations, so a ranking it produces is a statement about the representation and not only about the posts." if trm_reorders else "It ranks the columns in the same order across representations, so a ranking it produces survives the change of representation."}
Reports state the representation alongside the value for this reason, and the harness
files a report under the representation that produced it so that two cannot be mistaken
for one another.</p>
{f'''<p class="note">The subject-matter measure is a second clear case. Measured in the
word-count representation, {jsd_a} of the {len(llm_fns)} rewritten columns score no better
than the control that was handed the wrong topic entirely. That reads as complete topical
failure, and inspection of the outputs contradicts it. The measure is computed over
groupings of the representation, so it appears to have been detecting machine authorship
as much as topic drift. Measured in the neural representations, none of them does. The
earlier reading was a property of the representation rather than of the posts.</p>''' if jsd_flips else ""}
"""
...
<dl class="metrics">
  <dt>Topic JSD is unreliable for generated text in the TF-IDF space</dt>
  <dd>Measured there it assigns the Claude zero-shot column
  {val(systems, "llm_rewrite_claude", "distributional.topic_jsd"):.3f} against
  {val(systems, "shuffle_control", "distributional.topic_jsd"):.3f} for the shuffle
  control, despite a source similarity of
  {val(systems, "llm_rewrite_claude", "semantic.source_similarity"):.3f} and preservation
  of the topic on inspection. The measure is computed over groupings fitted on authentic
  posts, so it plausibly detects machine origin as well as a change of subject. It is
  therefore not reported as a finding. Re-measured in the EmbeddingGemma space the same
  column scores {val(systems_alt, "llm_rewrite_claude", "distributional.topic_jsd"):.3f}
  against {val(systems_alt, "shuffle_control", "distributional.topic_jsd"):.3f} for the
  shuffle control, which is the ordering the outputs support; see
  <a href="embedding-space.html">the embedding-space page</a>. The following example
  illustrates the original discrepancy.</dd>
  ...
  <dd>The results reported here are measured in the offline TF-IDF representation. Every
  comparison has been re-measured in {n_neural} further representations spanning two
  encoder families and, for one encoder, four sizes. {n_unanimous} of the {n_claims}
  comparisons reach the same verdict in every representation; {n_withdrawn} hold under the
  TF-IDF default and under no neural representation, and are consequently not results. Both
  of those rest on the triangle score, and the TF-IDF triangle score fails the harness's
  own baseline-separation test — it ranks the wrong-topic control second best rather than
  worst — which is why the neural triangle score is reported at 512 dimensions. The
  encoder may itself arrange posts by writing style, which is the principal risk identified
  for this project and which the harness mitigates only partially, so agreement across
  representations is supporting evidence rather than proof. The full comparison is on
  <a href="embedding-space.html">the embedding-space page</a>.</dd>
</dl>
...
"""
    ...
    (OUT / "embedding-space.html").write_text(
        page("embedding-space.html", "Embedding space", emb), encoding="utf-8"
    )
    ...
    print(f"Wrote {len(NAV)} pages and a stylesheet to {OUT}")
