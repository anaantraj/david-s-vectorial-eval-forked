...
def trm_per_cell(width=760, height=330):
    ...
    left, right, top, bottom = 78, 252, 74, 96
    pw, ph = width - left - right, height - top - bottom
    # The axis is derived from the data. It was previously fixed at 0.95, which
    # pushed the largest cell values off the top of the plot.
    observed = [
        v["trm"]
        for fn in fns
        for c in cells
        if (v := pc.get(f"{fn}.trm", {}).get(c)) and v.get("trm") is not None
    ]
    vmax = (max(observed) * 1.08) if observed else 1.0

    def py(v):
        return top + ph * (1 - v / vmax)

    out = [svg_open(width, height, "Per-cell TRM")]
    n_oracle = sum(1 for c in cells if pc.get("target_sample.trm", {}).get(c))
    out.append(text(0, 20, "The result holds in every cell that could be scored", "fig-title"))
    blk, _ = text_block(
        0, 38,
        f"Group-similarity score in each of the {len(cells)} cells with enough posts to "
        f"measure. Lower is better. The oracle can be scored in {n_oracle} of them, because "
        "it produces nothing on cells held out for generalisation.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
    out.append(f'<line class="axis" x1="{left}" y1="{top}" x2="{left}" y2="{top+ph}"/>')
    step = 0.2 if vmax <= 1.2 else 0.25
    gv = 0.0
    while gv <= vmax:
        out.append(f'<line class="grid" x1="{left}" y1="{py(gv)}" x2="{left+pw}" y2="{py(gv)}"/>')
        out.append(text(left - 10, py(gv) + 4, f"{gv:.2f}", "tick", "end"))
        gv += step
    ...
    ly = top
    for fn in fns:
        covered = sum(1 for c in cells if pc.get(f"{fn}.trm", {}).get(c))
        suffix = "" if covered == len(cells) else f"  ({covered}/{len(cells)})"
        out.append(f'<circle cx="{left+pw+22}" cy="{ly}" r="5.5" fill="{COLORS[fn]}"/>')
        out.append(text(left + pw + 34, ly + 4, rlabel(fn) + suffix, "key-lbl"))
        ly += 19
    out.append("</svg>")
    return "\n".join(out)
...
def rank_profile(systems, full, width=760):
    ...
    blk, _ = text_block(
        0, 38,
        "Each generated post is compared against two real posts. If the two sets matched, "
        "every band would be one third. A large third band means the generated posts are "
        "bunched in the middle of the real ones.",
        "fig-sub", width_px=width - 8,
    )
    out.append(blk)
...
{trm_per_cell()}
<p class="note">This score is computed over cosine distance in the TF-IDF and SVD
...
distance are not comparable with these. Claude zero-shot scores worse than the identity baseline in nine of the ten scored
cells, which establishes that the result is not caused by one unusual topic. The one
exception is the software development tools cell, where the two are close.</p>
